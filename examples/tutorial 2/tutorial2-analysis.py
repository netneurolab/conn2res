...
OUTPUT_DIR = 'E:/GitHub/conn2res/examples/results/T2-1000trials_0.0001gain/human' #os.path.join(PROJ_DIR, 'results', 'T2-1000trials_0.0001gain', 'human')
...
#
# for connectome in connectomes:
#     df_emp = pd.read_csv(os.path.join(OUTPUT_DIR, f'{connectome}_empirical_scores.csv')).reset_index(drop=True)
#     df_emp_avg = df_emp[['alpha', 'corrcoef']]
#     df_emp_avg = df_emp_avg.groupby('alpha').mean().reset_index()
#
#     df_null  = pd.read_csv(os.path.join(OUTPUT_DIR, f'{connectome}_null_scores.csv')).reset_index(drop=True)
