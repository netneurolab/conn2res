...
ALPHAS = [1.0]  # np.linspace(0, 2, 21)[1:]
...
for task_name in TASKS:
    ...
    df_subj.to_csv(os.path.join(OUTPUT_DIR, 'results.csv'), index=False)

    ###########################################################################
    # visualize performance curve
    # df_subj = pd.read_csv(os.path.join(OUTPUT_DIR, 'results.csv'), index_col=False)
